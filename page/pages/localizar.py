...
def combinar_dados():
    ...
    st.subheader('LOCALIZAR EXTINTORES AO ENDERECO')
    ...
    if st.button('LOCALIZAR EXTINTOR'):
        if dados.get(str(extintor_id)) and enderecos.get(str(endereco_id)):
            ...
            with open('localizado.txt', 'a') as arquivo:  # Modo 'a' para adicionar no final do arquivo
                # Calcular novo ID baseado no número de linhas existentes
                novo_id = sum(1 for _ in open('localizado.txt')) + 1
                linha = (f"{novo_id},{extintor_id},{dados[str(extintor_id)]['selo_inmetro']},"
